package Hashing;

import java.util.ArrayList;
import java.util.LinkedHashMap;

/**
 * Created by RajeshAatrayan|InterviewPreparation|Hashing|null.java| on Dec,2019
 * Happy Coding :)
 * <p>
 * Count Right Triangles
 * Given two arrays of integers A and B of size N each, where each pair (A[i], B[i]) for 0 <= i < N represents a unique point (x, y) in 2D Cartesian plane. Find and return the number of unordered triplets (i, j, k) such that (A[i], B[i]), (A[j], B[j]) and (A[k], B[k]) form a right angled triangle with the triangle having one side parallel to the x-axis and one side parallel to the y-axis. Note: The answer may be large so return the answer modulo (10^9 + 7).
 * Input Format
 * The first argument given is an integer array A.
 * The second argument given is the integer array B.
 * Output Format
 * Return the number of unordered triplets that form a right angled triangle modulo (10^9 + 7).
 * Constraints
 * 1 <= N <= 100000
 * 0 <= A[i], B[i] <= 10^9
 * For Example
 * Input 1:
 * A = [1, 1, 2]
 * B = [1, 2, 1]
 * Output 1:
 * 1
 * <p>
 * Input 2:
 * A = [1, 1, 2, 3, 3]
 * B = [1, 2, 1, 2, 1]
 * Output 2:
 * 6
 */
public class CountRightAngleTriangles {
    public int solve(ArrayList<Integer> x, ArrayList<Integer> y) {
        int mod = (int) (1e9 + 7);
        LinkedHashMap<Integer, Integer> forx = new LinkedHashMap<>();
        LinkedHashMap<Integer, Integer> fory = new LinkedHashMap<>();
        int n = x.size();
        for (int i = 0; i < n; i++) {
            forx.put(x.get(i), forx.getOrDefault(x.get(i), 0) + 1);
            fory.put(y.get(i), fory.getOrDefault(y.get(i), 0) + 1);
        }

        long count = 0;
        for (int i = 0; i < n; i++) {

            count = (count % mod + (((forx.get(x.get(i)) - 1) % mod * (fory.get(y.get(i)) - 1) % mod) % mod) % mod) % mod;
        }
        return (int) count;
    }
}

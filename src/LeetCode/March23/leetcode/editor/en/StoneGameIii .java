 ...
 * stones arranged in a row, and each stone has an associated value which is an integer
 ...
 *
 *
 * Related Topics Array Math Dynamic Programming Game Theory 👍 1480 👎 38
 */
...
class StoneGameIii {
    ...
    class Solution {
        ...
        private int recur(int index, int[] stoneValue) {
            if (index >= stoneValue.length) return 0;
            if (dp[index] != null) return dp[index];
            int sum = 0;
            int max = Integer.MIN_VALUE;
            for (int i = 0; i < 3; i++) {
                if (index + i >= stoneValue.length) break;
                sum += stoneValue[index + i];
                max = Math.max(max, sum - recur(index + i + 1, stoneValue));
            }
            dp[index] = max;
            return max;
        }

 ...
 *
 *
 * Related Topics Tree Depth-First Search Breadth-First Search 👍 2842 👎 187
 */
...
class TimeNeededToInformAllEmployees {
    ...
    class Solution {
        public int numOfMinutes(int n, int headID, int[] manager, int[] informTime) {
            ...
            return recur(headID, manager, informTime, list);

        }

        private int recur(int headID, int[] manager, int[] informTime, ArrayList<ArrayList<Integer>> list) {

            int max = 0;
            for (int x : list.get(headID)) {
                max = Math.max(recur(x, manager, informTime, list), max);
            }

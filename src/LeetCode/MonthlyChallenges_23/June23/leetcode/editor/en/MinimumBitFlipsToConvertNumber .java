 ...
 *
 *
 * Related Topics Bit Manipulation 👍 537 👎 12
 */
...
class MinimumBitFlipsToConvertNumber {
    ...
    class Solution {
        public int minBitFlips(int start, int goal) {
            int startBits = (int) ((Math.log(start) / Math.log(2)) + 1);
            int endBits = (int) ((Math.log(goal) / Math.log(2)) + 1);
            int maxBits = Math.max(startBits, endBits);
            int i = 0;
            int ans = 0;
            while (i < maxBits) {
                int sIthBit = (start & (1 << i)) != 0 ? 1 : 0;
                int eIthBit = (goal & (1 << i)) != 0 ? 1 : 0;
                if (sIthBit != eIthBit) {
                    ans++;
                }
                i++;

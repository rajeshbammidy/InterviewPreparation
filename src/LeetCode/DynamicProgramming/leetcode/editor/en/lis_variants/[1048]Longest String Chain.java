package LeetCode.DynamicProgramming.leetcode.editor.en.lis_variants;
 ...
 *
 *
 * Related Topics Array Hash Table Two Pointers String Dynamic Programming 👍 5557
 * 👎 216
 */

/**
 * Approach: Unlike LIS where the answer for the current i depends on any smaller element before i-1, here the answer of i
 * depends only on the previous length string.
 */
    ...
    public int longestStrChain(String[] words) {
        ...
        for (String word : words) {
            ...
            for (int i = 0; i < word.length(); i++) {
                ...
                String stringWithMissingChar = word.substring(0, i) + word.substring(i + 1);
                maxCount = Math.max(maxCount, dp.getOrDefault(stringWithMissingChar, 0) + 1);

            }
            dp.put(word, maxCount);
